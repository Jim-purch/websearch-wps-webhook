            ...
            {isOpen && (
                <div className="p-6 pt-0 border-t border-transparent">
                    {isLoading ? (
                        ...
                        <>
                            <div className="space-y-3 mb-3 mt-1.5">
                                {Object.entries(tablesByToken).map(([tokenId, group]) => {
                                    const isGsheet = group.tables[0]?.isGoogleSheets
                                    return (
                                        <div key={tokenId} className="border border-[var(--border)] rounded-md p-3 bg-[var(--card-bg)]">
                                            <h4 className="text-xs font-semibold text-[#eab308] mb-2 flex items-center justify-between border-b border-[var(--border)] pb-1.5">
                                                ...
                                                {isGsheet && (
                                                    <label
                                                        className="flex items-center gap-1.5 cursor-pointer text-[11px] font-normal text-indigo-400 select-none hover:text-indigo-300 transition-colors"
                                                    ...
                                                    >
                                                        <input
                                                            ...
                                                            className="w-3 h-3 accent-indigo-500 rounded border-[var(--border)] bg-[var(--card-bg)]"
                                                        />
                                                        <span>强制刷新缓存</span>
                                                    </label>
                                                )}
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {table.isGoogleSheets && (
                                                                table.cacheTime ? (
                                                                    <span className="text-[10px] text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-1 py-0.2 rounded flex items-center gap-0.5" title="当前缓存时间，搜索将快速从该缓存检索">
                                                                        📅 缓存: {table.cacheTime}
                                                                    </span>
                                                                ) : (
                                                                    <span className="text-[10px] text-amber-500 bg-amber-500/10 border border-amber-500/20 px-1 py-0.2 rounded flex items-center gap-0.5" title="尚未加载缓存，首次搜索或点击加载列信息将触发缓存拉取">
                                                                        无缓存
                                                                    </span>
                                                                )
                                                            )}
